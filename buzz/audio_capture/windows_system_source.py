...
class WindowsSystemAudioSource(AudioSource):
    ...
    def start(
        ...
        with self._condition:
            if self._starting or self._active or self._process is not None:
                raise AudioSourceError("System audio source is already active")
            self._generation += 1
        ...
        command = [str(self._helper_path), "--mode", "system"]
        ...
        try:
            ...
            raise AudioSourceError(
                f"Could not start Windows system audio capture: {exc}"
            ) from exc
        ...
        if process.stdin is None or process.stdout is None or process.stderr is None:
            try:
                ...
                self._cancel_unlaunched_start(generation)
            raise AudioSourceError("Windows system audio helper pipes are unavailable.")

        try:
            reader_thread = threading.Thread(
                target=self._read_stdout,
                args=(process, generation, startup_event, delivery_enabled),
                name="windows-system-audio-reader",
            )
            stderr_thread = threading.Thread(
                target=self._drain_stderr,
                args=(process, process.stderr, generation),
                name="windows-system-audio-stderr",
            )
        except Exception as exc:
            try:
                self._cleanup_unowned_process(process)
                ...
                logging.exception(
                    "Windows system audio cleanup failed after thread creation failure"
                )
            finally:
                self._cancel_unlaunched_start(generation)
            raise AudioSourceError(
                f"Could not create Windows system audio worker threads: {exc}"
            ) from exc
        accepted = False
        ...
        if not accepted:
            self._cleanup_unowned_process(process)
            raise AudioSourceError("Windows system audio capture startup was canceled.")
        ...
        if thread_start_error is not None:
            try:
                ...
                logging.exception(
                    "Windows system audio cleanup failed after thread startup failure"
                )
            raise AudioSourceError(
                f"Could not start Windows system audio worker threads: {thread_start_error}"
    ...
    def _read_stdout(
        ...
        try:
            ...
            if not stopping:
                if carry:
                    error = AudioSourceError(
                        "Windows system audio transport ended with an incomplete sample."
                    )
                else:
            ...
            error = AudioSourceError(f"Windows system audio transport failed: {exc}")
            if not startup_event.is_set():
                self._set_startup_error(
                    error,
                    process,
    ...
    def _deliver(
        ...
        if callback_error is not None:
            self._report_runtime_failure(
                AudioSourceError(f"System audio callback failed: {callback_error}"),
                process,
                generation,
            )
    ...
    def _drain_stderr(
        ...
        try:
            ...
            logging.debug("Windows system audio stderr drain ended: %s", exc)
    ...
    def _report_runtime_failure(
        ...
        try:
            ...
            logging.exception(
                "Could not signal Windows system audio helper after runtime failure"
            )
        logging.error("Windows system audio capture failed: %s", error)
        if callback is not None:
            try:
                callback(error)
            except Exception:
                logging.exception("Windows system audio error callback failed")
    ...
    def _shutdown(self) -> None:
                ...
                try:
                    ...
                    if thread.is_alive() and shutdown_error is None:
                        shutdown_error = AudioSourceError(
                            f"Windows system audio {name} thread did not stop."
                        )
                except Exception as exc:
                    if shutdown_error is None:
                        shutdown_error = exc
        ...
        if shutdown_error is not None:
            ...
            raise AudioSourceError(
                f"Could not stop Windows system audio capture: {shutdown_error}"
            ) from shutdown_error
    ...
    def _terminate_process(self, process: subprocess.Popen) -> None:
            ...
            try:
                ...
                if primary_error is None:
                    primary_error = AudioSourceError(
                        "Windows system audio helper did not exit after it was killed."
                    )
            except Exception as exc:
                if primary_error is None:
                    primary_error = exc
        ...
        if primary_error is not None:
            if isinstance(primary_error, AudioSourceError):
                raise primary_error
            raise AudioSourceError(
                f"Windows system audio helper shutdown failed: {primary_error}"
            ) from primary_error
    ...
    def _complete_failed_start(self, process: subprocess.Popen) -> None:
        ...
        try:
            ...
            logging.exception("Windows system audio startup cleanup failed")
    ...
    def _cleanup_unowned_process(self, process: subprocess.Popen) -> None:
        ...
        if cleanup_error is not None:
            if isinstance(cleanup_error, AudioSourceError):
                raise cleanup_error
            raise AudioSourceError(
                f"Could not clean up Windows system audio helper: {cleanup_error}"
            ) from cleanup_error
    ...
    def _helper_error(
        ...
        *,
        ...
        if native_detail:
            logging.error("Windows system audio helper diagnostics: %s", native_detail)
    ...
    def _validate_header(cls, header: bytes) -> None:
        ...
        if magic != cls._MAGIC:
            raise AudioSourceError("Windows system audio helper sent an invalid handshake.")
        if version != cls._PROTOCOL_VERSION or header_size != cls._HEADER.size:
            raise AudioSourceError(
                "Windows system audio helper uses an unsupported protocol version."
            )
        if sample_rate != cls.SAMPLE_RATE:
            raise AudioSourceError("Windows system audio helper did not provide 16 kHz audio.")
        if channels != 1:
            raise AudioSourceError("Windows system audio helper did not provide mono audio.")
        if sample_format != cls._SAMPLE_FORMAT_FLOAT32_LE:
            raise AudioSourceError(
                "Windows system audio helper did not provide float32 audio."
            )
